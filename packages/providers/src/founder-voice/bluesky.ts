  ...
  try {
    ...
    for (const founder of input.targets.founders) {
      const searchPayload = await fetchBlueskyJson<BskySearchActorsResponse>(
        fetchFn,
        `${BLUESKY_SEARCH_URL}?q=${encodeURIComponent(founder.name)}&limit=5`,
        input.timeoutMs,
      );

      const actor = (searchPayload.actors ?? []).find((candidate) =>
        isMatchingFounderActor(candidate, founder.name, input.targets),
      );
