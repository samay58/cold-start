...
describe("split synthesize/verify units", () => {
  ...
  describe("verifyCardSynthesisDraft", () => {
    ...
    describe("how-it-wins verification", () => {
      function howItWinsFixtures() {
        const whyItMatters = { text: "Cartesia is building voice AI infrastructure. [c1]", citationIds: ["c1"] };
        const bullCase = { text: "Cartesia has public product evidence. [c1]", citationIds: ["c1"] };
        const bearCase = { text: "Durable differentiation is not fully proven. [c1]", citationIds: ["c1"] };
        const loud = { text: "The founders talk up enterprise pilots constantly. [c1]", citationIds: ["c1"] };
        ...
        const runningOne = {
          ...
          note: "Cartesia builds only voice models, nothing else. [c1]",
          citationIds: ["c1"]
        };
        const runningTwo = {
          strategy: "iteration" as const,
          meaning: "Iterates and changes quickly.",
          note: "Cartesia ships model updates on a weekly cadence. [c1]",
          citationIds: ["c1"]
        };
        const pair = {
          strategies: ["specialization", "iteration"] as const,
          note: "One narrow surface plus weekly shipping compounds faster than a broad platform can. [c1]",
          wrongIf: "The roadmap widens past voice.",
          citationIds: ["c1"]
        };
        const filedHowItWins = {
          status: "read" as const,
          sentence: "Cartesia wins by staying narrow and shipping faster than broader rivals.",
        ...
        const pairClaim = { text: pair.note, citationIds: pair.citationIds };
        return {
          whyItMatters,
          bullCase,
          bearCase,
      ...
      it("appends the running notes then the pair note after the emphasis claims and keeps a fully supported read", async () => {
        const card = await assembledCard();
        const {
          whyItMatters,
          bullCase,
        ...
        const verify = vi.fn(async (claims: unknown[]) => {
          // 3 synthesis claims, then the 3 emphasis claims, then the 2 running notes and the pair
          // note: the how-it-wins claims sit strictly after everything the emphasis read
          // contributes, which is exactly what howItWinsOffset has to account for.
          expect(claims).toHaveLength(9);
          expect(claims[6]).toEqual(runningClaimOne);
          expect(claims[7]).toEqual(runningClaimTwo);
          expect(claims[8]).toEqual(pairClaim);
          return [
            { claimIndex: 0, ...whyItMatters, status: "supported" as const },
            { claimIndex: 1, ...bullCase, status: "supported" as const },
            { claimIndex: 2, ...bearCase, status: "supported" as const },
            ...
            { claimIndex: 4, ...read, status: "supported" as const },
            { claimIndex: 5, text: quiet, citationIds: [], status: "unsupported" as const },
            { claimIndex: 6, ...runningClaimOne, status: "supported" as const },
            { claimIndex: 7, ...runningClaimTwo, status: "supported" as const },
            { claimIndex: 8, ...pairClaim, status: "supported" as const }
          ];
        });
        ...
        const result = await verifyCardSynthesisDraft(
          ...
          draft,
          { verify, synthesisRequired: true },
          { emphasisRead: filedEmphasisRead, howItWins: filedHowItWins }
        );

        expect(verify).toHaveBeenCalledTimes(1);
        expect(result.emphasisRead).toEqual(filedEmphasisRead);
        expect(result.howItWins).toEqual(filedHowItWins);
        expect(result.howItWinsDropReason).toBeUndefined();
      });

      it("drops the pair and reports pair-dropped when only the pair note is unsupported", async () => {
        const card = await assembledCard();
        const {
          whyItMatters,
          bullCase,
        ...
        const verify = vi.fn(async () => [
          ...
          { claimIndex: 5, ...pairClaim, status: "unsupported" as const }
        ]);
        ...
        expect(result.howItWins).toMatchObject({ status: "read", pair: null });
      ...
      it("degrades to nothing_stands_out when both running notes drop", async () => {
        ...
        const verify = vi.fn(async () => [
          ...
          { claimIndex: 2, ...bearCase, status: "supported" as const },
          { claimIndex: 3, ...runningClaimOne, status: "unsupported" as const },
          { claimIndex: 4, ...runningClaimTwo, status: "contradicted" as const },
          { claimIndex: 5, ...pairClaim, status: "supported" as const }
        ]);
      ...
      it("without the extra the verify payload and result shape are unchanged", async () => {
        const card = await assembledCard();
        const { whyItMatters, bullCase, bearCase, draft } = howItWinsFixtures();
        const verify = vi.fn(async (claims: unknown[]) => {
          expect(claims).toHaveLength(3);
          return [
            { claimIndex: 0, ...whyItMatters, status: "supported" as const },
        ...
        expect(result).not.toHaveProperty("howItWins");
        expect(result).not.toHaveProperty("howItWinsDropReason");
      });
    });
  });
