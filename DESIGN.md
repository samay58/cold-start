...
The investor read (`apps/extension/src/research/InvestorReadCard.tsx`) is a compact category packet, not one continuous memo and not a dashboard row. It is one calm mineral reading surface with five named ideas, not five cards competing with the Research stack beneath it and not a faux paper dossier. It runs on exactly five type roles. Every rule in the packet maps to one row below; a new UI moment gets spacing, weight, or a mark before it gets a sixth size.

| Role | Spec | Use |
|---|---|---|
| Lede | 13.25px / 1.5 / IBM Plex Sans 470 | The Why care thesis, set as reading text rather than a second headline |
| Section label | 13.5px / 1.2 / IBM Plex Sans 650, sentence case | "Why care", "What must be true", "What could break", "Why now", "What to learn next" |
| Claim | 13px / 1.5 / IBM Plex Sans 450 | Bull, bear, timing, and question bodies |
