...
The crown (`apps/extension/src/research/HowItWinsEdge.tsx`) carries the How it wins read on the plate's top edge, above the header. It is ink only. A pinned mark takes 2px more depth and a heavier wall, never the seal color. At rest it shows a label row with the count on the right, a hairline rule notched once for each strategy the company runs, a dashed notch for each one it has not taken yet, one bracket under the two that work together, and a sentence that carries the read on its own for a reader who never hovers. The eighty ticks behind the rule, one per strategy in the shared vocabulary, stay invisible until the pointer enters, and the readout swaps to the name under the pointer as it moves. A click pins the mark under the pointer and opens its note below the sentence on the roles above, with a pinned receipt in the corner and a Wrong if line for the pair. The note is absolutely positioned, so it never covers the sentence and the plate keeps its height through every hover and every pin.
