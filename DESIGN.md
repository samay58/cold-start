...
> Current visual source of truth. Last updated 2026-08-02 for the Investor Lens reading surface within the Catalogue Card direction, shared by the public `/c/{slug}` card and the browser extension. The Signal Ledger system that preceded it is archived at `docs/brand/archive/signal-ledger-era-2026-05.md`; the 2026-05-26 Ray Gun pass at `docs/brand/archive/raygun-era-2026-05-27.md`; the earlier parchment system at `docs/brand/archive/parchment-era-2026-05.md`. If code disagrees with this file, treat this file as the intended direction and update code deliberately.
...
The investor read (`apps/extension/src/research/InvestorReadCard.tsx`) is a compact category packet, not one continuous memo and not a dashboard row. It is one calm mineral reading surface with four named ideas, not four cards competing with the Research stack beneath it and not a faux paper dossier. It runs on exactly five type roles. Every rule in the packet maps to one row below; a new UI moment gets spacing, weight, or a mark before it gets a sixth size.

| Role | Spec | Use |
|---|---|---|
| Lede | 13.25px / 1.5 / IBM Plex Sans 470 | The Why care thesis, set as reading text rather than a second headline |
| Section label | 13.5px / 1.2 / IBM Plex Sans 650, sentence case | "Why care", "The case", "What to learn next", "Pay attention to" |
| Claim | 13px / 1.5 / IBM Plex Sans 450 | Bull, bear, emphasis, and question bodies |
...
The packet contains four rows in a fixed order: Why care, The case, What to learn next, and Pay attention to. Why care carries the thesis and closes on the timing beat when the record supports one, so timing has no row of its own. The case holds both sides of the argument in one body, Bull over Bear, each led by its own mark. How it wins sits on the packet's edge rather than in the row list. The rows are not numbered because they are not a process and the number adds no meaning. One outer 6px mineral plate carries the header, rows, reading content, source posture, and update receipt. The research-layer wrapper around it stays borderless and transparent; the Lens must never sit inside a second decorative card. Internal hairlines provide structure, and the rows never become separate filled cards. Why care opens when a live or cached Lens arrives. At most one row is open; selecting the open row collapses the packet fully. Every closed row keeps a readable one-line preview, so the entire read is scannable before the reader commits to the supporting prose. A long Why care lede shows four lines and keeps the rest behind a quiet Read full disclosure.
...
The crown (`apps/extension/src/research/HowItWinsEdge.tsx`) carries the How it wins read on the plate's top edge, above the header. It is ink only. A pinned mark takes 2px more depth and a heavier wall, never the seal color. At rest it shows a label row with the count on the right, a hairline rule notched once for each strategy the company runs, a dashed notch for each one it has not taken yet, a muted solid outline for each one still unresolved, one bracket under the two that work together, and a sentence that carries the read on its own for a reader who never hovers. The eighty ticks behind the rule, one per strategy in the shared vocabulary, stay invisible until the pointer enters, and the readout swaps to the name under the pointer as it moves. An unresolved mark reads as the name plus a question mark. A click pins the mark under the pointer and opens its note below the sentence on the roles above, with a pinned receipt in the corner and a Wrong if line for the pair. The note is absolutely positioned, so it never covers the sentence and the plate keeps its height through every hover and every pin.

The person dossier (`apps/extension/src/shared/SharedTooltip.tsx`'s `DossierBody`) shares this scale rather than minting its own: identity (name + role) and the read sit at Claim, the identity line at weight 560 to read as a header without a sixth size; the email address is Meta; the observed/inferred badge, the basis line, the folded-in provenance citation mark, and the channels row are all Receipt. Nothing in the dossier is Lede or Section label. The expanded-description memo (`SharedTooltip`'s memo variant, behind the header summary's "(more)" affordance) borrows the same frame: a display-face title at the Section-label size over Claim-role body paragraphs, the dossier's close control and pin behavior, and vertical scroll inside the window; it never scrolls the panel horizontally.
...
Extension dark mode: the side panel ships a warm paper dark mode anchored on the Kyoto sumi night palette (ground #1B1612, aged off-white ink #E8DDC9, lifted dusty-lilac seal #BBA8DF, evidence hues retuned for a dark ground). It activates on OS dark; a manual Auto/Light/Dark control in settings always wins. White inset sheens become a faint warm amber edge lift, never glossy white. Full token map and rationale in `docs/theme-sources.md`; product spec in `docs/archive/product/extension-dark-mode-spec-2026-06-24.md`. The public web card stays light.
