...
| `HOW_IT_WINS_ENABLED`                                                     | Enables the How it wins read: the background `how-it-wins-read` Inngest function (judge, write, verify, store) that the analysis run dispatches after storing fresh synthesis, gated on the same thin-file check (default on; production is OFF as of 2026-08-25 pending verification of the streaming-transport repair; set `false` to roll back).                                       |
...
| `LLM_HOW_IT_WINS_MODEL`                                                   | Frozen writer model for the How it wins reasoning, edit, and fit passes. Falls back to the synthesis model chain when unset.                                                                                                                                                                                                                                                             |
| `LLM_HOW_IT_WINS_EDITOR_MODEL`                                            | Hostile-editor pass model, a different provider from the writer by design (default `deepseek/deepseek-v4-pro`). Any failure on this pass, transient or not, carries the writer's draft forward unchanged and is recorded in the trace as editorSkipped.                                                                                                                                                                                      |
