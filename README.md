...
| `HOW_IT_WINS_ENABLED`                                                     | Enables the How it wins read: the background `how-it-wins-read` Inngest function (judge, write, verify, store) that the analysis run dispatches after storing fresh synthesis, gated on the same thin-file check (default on; production has run it since the 2026-08-25 repair deploy; set `false` and redeploy to roll back).                                       |
| `HOW_IT_WINS_REFINEMENT`                                                  | Controls the critic and adjudication passes that run after the all-80 judge (default on). Set `off` to skip both: the judge returns straight from its global judgment, saving about $0.30 and 60s per run. No deploy needed either way.                                                                                                                                                 |
