...
| `XAI_API_KEY`                                                             | Optional. Enables the `xai_x_search` founder-voice lane for the emphasis read (the sixth Investor Lens category, "Pay attention to"). Without it, that lane alone degrades to zero cost and zero items; the other four founder-voice lanes are unaffected.                             |
...
| `LLM_HOW_IT_WINS_EDITOR_MODEL`                                            | Hostile-editor pass model, a different provider from the writer by design (default `deepseek/deepseek-v4-pro`). A non-transient failure on this pass carries the writer's draft forward unchanged.                                                                                                                                                                                      |
