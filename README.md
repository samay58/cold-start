...
| `LLM_EXTRACT_FALLBACK_MODEL`                                               | Alternate-provider extraction model, tried after `LLM_EXTRACT_MODEL` and before `ANTHROPIC_MODEL`. Must be a different provider. Set `off` to disable it.                                                                                 |
...
| `OPENROUTER_API_KEY` and `OPENROUTER_BASE_URL`                            | Credentials for the OpenRouter provider, used by provider-prefixed model names such as `openrouter/google/gemini-2.5-flash`.                                                                                                              |
| `DEEPINFRA_API_KEY` and `DEEPINFRA_BASE_URL`                              | Credentials for the DeepInfra provider. Kept available for diagnostics; not currently selected in production after capacity failures.                                                                                                    |
...
| `GITHUB_TOKEN`                                                            | Optional GitHub personal access token for the free commit-email reachability layer (5,000 req/hr vs 60 unauthenticated) and the `github_author_activity` founder-voice lane. Both degrade gracefully when absent (lower rate limit; the lane still runs unauthenticated). Read-only public scope is sufficient.                            |
| `XAI_API_KEY`                                                             | Optional. Enables the `xai_x_search` founder-voice lane for the emphasis read (the "Pay attention to" row of the Investor Lens). Without it, that lane alone degrades to zero cost and zero items; the other four founder-voice lanes are unaffected.                             |
| `XAI_XSEARCH_MODEL`                                                       | Optional override for the xAI model the `xai_x_search` lane calls (default `grok-4.20-non-reasoning-latest`). Only takes effect when `XAI_API_KEY` is also set.                                                                          |
...
| `EMPHASIS_READ_ENABLED`                                                   | Enables the emphasis read: the founder-voice fetch and the `emphasis_read` LLM stage inside analysis runs that clear the thin-file check (default on). Per-stage model routing follows the same `LLM_*_MODEL` mechanism as every other stage, via `LLM_EMPHASIS_READ_MODEL`. Set false to skip both Inngest steps, no lane fetch and no model call.                                  |
| `HOW_IT_WINS_ENABLED`                                                     | Enables the How it wins read: the background `how-it-wins-read` Inngest function (judge, write, verify, store) that the analysis run dispatches after storing fresh synthesis, gated on the same thin-file check (default on in code; set `false` and redeploy to roll back; verify current production behavior through a current run trace).                                       |
| `HOW_IT_WINS_JOB_BUDGET_USD`                                              | Required maximum USD spend for one How it wins job plus its single manual retry, at most 10. Missing configuration or an unsupported pricing entry rejects admission.                                                                     |
| `HOW_IT_WINS_RETRY_ENABLED`                                               | Enables new manual retries on a failed How it wins job. Keep `false` during deployment; enable only after the bounded canary passes.                                                                                                       |
| `HOW_IT_WINS_REFINEMENT`                                                  | Controls the critic and adjudication passes that run after the all-80 judge (default on). Set `off` to skip both: the judge returns straight from its global judgment, saving about $0.30 and 60s per run. A Vercel change applies on the next deployment.                                                                                                                               |
| `LLM_HOW_IT_WINS_JUDGE_MODEL`                                             | Model for the all-80 strategy judge. Falls back to `LLM_HOW_IT_WINS_MODEL` when unset. Must be a different provider from `LLM_HOW_IT_WINS_EDITOR_MODEL`; the judge refuses at construction otherwise. Judgments are memoized in `how_it_wins_judgments`, keyed by evidence-packet, prompt, and vocabulary hashes.                                                                     |
| `LLM_HOW_IT_WINS_MODEL`                                                   | Frozen writer model for the How it wins read, and the judge model when `LLM_HOW_IT_WINS_JUDGE_MODEL` is unset. Falls back to the synthesis model chain when unset.                                                                                                                                                                                                                                                             |
| `LLM_HOW_IT_WINS_EDITOR_MODEL`                                            | Critic model for the judge's refinement pass, a different provider from the judge by design (default `deepseek/deepseek-v4-pro`). A critic or adjudication failure keeps the judge's global verdict and is recorded under `refinement` on the judgment.                                                                                                                                                                                      |
| `LLM_HOW_IT_WINS_FALLBACK_MODEL`                                          | Optional alternate-provider continuity for the How it wins passes, same pattern as `LLM_*_FALLBACK_MODEL` above.                                                                                                                                                                                                                                                                         |
| `INNGEST_HOW_IT_WINS_CONCURRENCY`                                         | Optional concurrency cap for the `how-it-wins-read` background function, same pattern as the `INNGEST_CARD_ENRICHMENT_CONCURRENCY` / `INNGEST_CONTACT_ENRICHMENT_CONCURRENCY` caps on the two enrichment workers (`.env.example`, `docs/deployment.md`). Unset means no cap.                                                                                                          |
...
EXTRACTION_EVIDENCE_BUDGET_CHARS=45000
```
...
Firefox uses a window-scoped sidebar instead of Chrome's per-tab side panel: the sidebar shows the last researched company in that window, the toolbar click is what researches the current tab, and a quiet hint appears when the visible tab no longer matches the researched one.

For local development, build with the local origin and run in a scratch profile:
...
Local builds relax the extension-pages CSP because Firefox's default upgrades `http://localhost` fetches to https; production builds keep Mozilla's default. Releases go through `npm run package:firefox` (deterministic artifact plus manifest inspection), `npm run sign:firefox` (AMO unlisted signing with the reviewer source package attached), and `npm run release:firefox -- <signed.xpi>` (publishes the XPI and stamps `updates.json` for self-hosted updates); `docs/deployment.md` documents the lane.
