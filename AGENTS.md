...
- Generation has two profile tiers: `basics` (sourced public card, can be cached) and `analysis` (extension-gated synthesis). `mode` records the tier; `jobKind` records the exact job (`basics`, `analysis`, or `section:<id>`). Initial extension generation starts as `basics`; analysis and gated sections require an existing usable profile.
