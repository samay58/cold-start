...
- `apps/web`: Next.js 15 App Router app (Tailwind v4, React 19). Hosts `/c/{slug}`, `/privacy`, `/alpha` (the friend-alpha invite page), `/api/cards/{slug}`, `/api/extension/cards/{slug}`, `/api/extension/bootstrap`, `/api/generate`, `/api/inngest`, and `/api/alpha/*` (`events`, `invite/inspect`, `invite/redeem`, `invite/status`).
...
- `packages/db`: Drizzle ORM and Postgres repository layer. Local Postgres uses host port `55432`. `src/repositories/alpha.ts` (migration 0009, six tables: invites, installations, allowances, allowance ledger, run requests, events) is the friend-alpha invite and allowance system; `reserveAlphaRunRequest` and `settleAlphaRunRequest` call PL/pgSQL functions (`reserve_alpha_run_request`, `settle_alpha_run_request`) that take advisory locks and debit or refund profile and Lens allowances atomically.
