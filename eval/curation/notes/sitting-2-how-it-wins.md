...
- The [ChatGPT framework-mapping prompt](../../../docs/archive/specs/2026-08-21-how-it-wins-framework-mapping-chatgpt-prompt.md) is saved for downstream ingestion.
...
- The smaller decision screen is ready for its spend gate. The frozen seed selected Hebbia, August, and Neko Health before their judgments were inspected. Each will run through monolith, four bundles, and 13 groups, for nine fresh arms. The preflight passed 10 core tests, 26 judge tests, 42 benchmark tests, all workspace typechecks, lint, and a dry run with 30 fake arms, 250 fake calls, and zero providers. The private pilot record, decisions, traces, and spend reconcile to $47.165909. Four valid pilot answers each retain all 80 strategy rows, the review remains topology-blind, and the old 30-arm rerun stays canceled. Next is exact cap approval for the nine-arm screen.
