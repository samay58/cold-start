...
2. **Polish gate (Fable).** Fable taste pass on the assembled surfaces via screenshots before anything ships.
...
5. **Production, BEFORE Samay tests (his explicit instruction 2026-07-05).** `npm run db:migrate:production` applies 0008 BEFORE traffic hits the new code (prod-migration-drift lesson: Vercel deploys never run Neon migrations; verify applied state, do not assume). Deploy web; verify the deployed response carries contract version `2026-07-05.assembly-clippings-person-reads-v1`. Then extension: `npm run build`, reload `apps/extension/dist` unpacked in chrome://extensions, reopen the side panel. Decide `LLM_PERSON_READ_MODEL` in Vercel (unset = synthesis chain fallback) before the first production contact-enrichment run.
6. **Samay's live test on virgin companies (the real sign-off, after the deploy).** Hand Samay a test kit of 6 to 8 startups relevant to his work (AI-native, the kind he would research) that have NEVER been generated before: verified absent from the production `cards` table (read-only slug check), and not in `eval/golden-companies.seed.json` or the QA fixture set, so every one exercises cold generation end to end with Reduce Motion OFF: whisper, seal, clippings arriving, dossier hovercards, and a person read once contact enrichment lands. His verdict here is the quality bar for the whole overhaul.
7. **Ledger final report.** Totals by tier, execution-phase Fable share, counterfactual, gate results, Samay's verdict. Doctrine promotion decision (pointer lines into phoenix and other projects) made with Samay only if all bars pass.
