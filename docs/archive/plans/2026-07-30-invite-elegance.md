Status: shipped 2026-07-30 (a5d6063).

# Invite Elegance Implementation Plan

Status note: `SECURITY.md` and migration 0016 supersede this plan's preview-authorization and abuse-control steps.
...
**Spec:** `docs/archive/specs/2026-07-30-alpha-invite-elegance-design.md`. Read it first.
...
- Live-test incident (2026-07-30 night): the extension's external-message gate (`isTrustedAlphaInviteSender`) pinned the `/alpha` path, so `/i/{slug}` pages could not probe or connect any shipped build; the plan's "four entry points" missed this fifth one. Widened to `/i/` (409a63e). Two prod-only walls fell the same hour: the July 24 store build's client contract needed `ALPHA_SUPPORTED_PREVIOUS_CLIENT_CONTRACTS=2026-07-24.alpha-principal-v1`, and `CHROME_WEB_STORE_URL` plus the store extension ID were never in Vercel env. Store-build testers on old builds connect via the legacy `/alpha#<code>` link until a store build carrying 409a63e ships.
