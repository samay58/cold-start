Status: shipped 2026-07-22 (12046c3).
...
5. **Phase 3: DONE (2026-07-22).** Docked person dossier shipped; occlusion regression proven RED against pre-phase code at two independent levels.
6. **Phase 4: SHIPPED (2026-07-22, motion review approved).** Step split, watchable wait, and the re-fetch lever (off, default `full`) merged and deployed. Open tails: the skip-fresh promotion decision (additional prod-parity shadow pairs being gathered; flip is a quiet-window operation), and the two-week `measure:analysis-latency` measurement against the p50 60s / p90 90s bar.
...
**Design authority:** `docs/archive/specs/2026-07-20-investor-lens-overhaul-design.md` (the approved decision record). Where this plan and the spec disagree, the spec wins; flag the conflict instead of guessing.
...
- [x] `npm run check` green from the main tree.
- [x] Local closed-loop QA (playbook: `docs/qa/generation-trace-and-production-qa.md`): run analysis against moonshot, generaltranslation, timescaledb through the local stack. Each must land in read-or-honest-withheld; zero silent completions. Record outcomes here.
- [x] Confirm in local DB: withheld run trace carries `synthesis.gate`; card carries `synthesisWithheld`; re-click without changes returns instantly with no new `generation_runs` row.

**Finding (2026-07-21, gate QA):** Closed-loop QA passed for moonshot.ai, generaltranslation.com, timescaledb.org (agent caught two wrong domains in the task brief), and oboe.com: all landed as reads, zero silent completions, $1.32 total spend. timescaledb, the 3x repeat silent-withhold victim, now reads with a `no-funding-evidence` advisory. The withheld branch was exercised deterministically by raising the env floor to 50: record written, trace gate blocked, verified in psql. That exercise exposed the phase's one Critical: the free pre-check compared persist-time `updated_at` against assembly-time `synthesisWithheld.at`, unsatisfiable by ~9ms, so every withheld re-click queued a paid run; the Opus whole-branch review found the same seam statically. Fixed in `5606b73` by comparing re-derived evidence signals against the frozen record (no timestamps, no extra DB read); re-review verdict Ready to Merge (the same-count-swap limitation is provably harmless: the gate is a pure function of the two compared counts); live re-QA confirmed 200-withheld with no new run row, stable across repeats, forceRefresh still queues. Side discovery filed as issue #10: a gate-blocked re-run wipes a previously good read (reachable via forceRefresh onto a degraded evidence pool or the new stale-in-run path); product decision pending. audit:deps advisories (axios, brace-expansion, js-yaml) cleared by version bumps in `eb699a1`, no allowlisting. Slop gate over all shipped copy: clean.
- [x] Deploy (`docs/deployment.md`), rebuild the extension against the deployed origin, verify the deployed withheld flow on one slug. Note: Vercel deploys do not run Neon migrations (none needed this phase; confirm `drizzle` folder untouched).
- [x] Update the lens-silent-gate memory/known-issues note: the gate is no longer silent as of this deploy.

**Done (2026-07-21, Phase 1 shipped):** Merged to main as a fast-forward (`e00c6d9..137d080`) and pushed; Vercel production deploy for `137d080` confirmed serving contract `2026-07-20.synthesis-withheld-v1` on the public origin within a minute of READY. Drizzle folder untouched, so the no-migration deploy is safe by construction. Prod invariants verified from here: public card JSON carries neither `synthesis` nor `synthesisWithheld`; unauthed generate POST returns 403. Extension rebuilt against the deployed origin into `apps/extension/dist`. The one step that needs a human browser: reload the unpacked extension in chrome://extensions and click analysis on a thin company (casaphq, daytona, or inkeep per the gate study) to see the deployed withheld card end to end; the same flow passed live QA on the local stack, including the free re-click and paid retry. Lens-silent-gate memory updated to RESOLVED.
...
- [x] **Step 2: Confirm fail.**
- [x] **Step 3: Implement against the gallery.** Iterate with `qa:extension:gallery` screenshots against `read-full`, `read-sparse`, `withheld-advisory`; cite `gold-standard-references.md` choices in the PR/commit body. Check narrow width (360px) and the Playwright long-text guard.
- [x] **Step 4: Tests pass + gallery screenshots captured for review.**
- [x] **Step 5: Update DESIGN.md with the scale table. Update CLAUDE.md/AGENTS.md file pointers (InvestorReadCard now its own file). Commit.**

**Done (2026-07-21):** Commits `542fad3` (extraction + memo) and `0233fe9` (docs). Extension 252/252 incl. audit:css, sidepanel-ui Playwright 53/53, full monorepo test and `check` green per the implementer with the load-bearing subset re-run by the reviewer. Opus review: Spec PASS, Quality Approved, Taste Approved; the "If true." / "It breaks if." inline em-prefix (implementer judgment call) was explicitly endorsed as what keeps the surface a memo rather than a label rail. Riding minors landed: shared `synthesis-advisory-copy.ts` exhaustively typed against core unions (new member breaks the build, verified), retry button in-flight disabled affordance, withheld surface derives all copy from the frozen record while the read posture derives live via the extracted `synthesisAdvisoriesFromSignals` (core refactor verified bit-identical by the reviewer). Accepted letter-deviation, recorded deliberately: CSS styles the marks via `data-mark` set from side semantics rather than a `data-side` selector; the underlying defect (bull/bear visually identical) is resolved, so no churn on a taste-approved surface. Minors for the final review: non-discriminating post-click content assertion in the rewritten timing test; collapsed disclosure content stays in the a11y tree; `.cs-investor-read-more` lacks tabular-nums; defensive dead-at-rest frozen-record branch in the read posture. Taste watch-items for Samay's gallery review: the slashed mark's weight at a glance, and the posture line rendering after the Filed stamp.
...
- [x] **Step 1: Failing test: exit-state coverage.** Per repo rule, record what happens when the exiting element is the final item: running card exits while result mounts; withheld card exits to trigger on retry. Assert both transitions render without orphaned nodes (jsdom-level: presence/absence after animation completion with skipAnimations).
- [x] **Step 2: Confirm fail. Step 3: Implement. Step 4: Pass + gallery re-screenshot (motion reviewed live, not in stills; note for review session). Step 5: Commit.**

**Done (2026-07-21):** Commits `156ad0e` (motion) and `8db9c12` (review fixes). Extension 255/255 incl. audit:css and the new exit-state cases, typecheck clean, gallery 6/6. Choreography as reviewed by Opus with independently derived numbers: 200ms slot crossfade (100ms reduced), stagger starts 0/140/280/420ms on `commitSpring` (zeta ~0.908, overshoot ~0.11%, no visible bounce), visible settle ~540-590ms, reduced motion collapses to one 150ms opacity fade. Review found and the fix pass closed two Importants: the slot swap was a stacked cross-dissolve that ballooned the container (fixed with `mode="popLayout"` plus `position: relative` on the slot; live-measured 79px to 1169px to 1080px before, no balloon after) and the pre-existing reduced-motion freeze on the running dot (now the shared `cs-reduced-breathe` pattern; reduction, never a freeze). Re-review verdict Ready; exit-state tests confirmed non-vacuous under popLayout. For the live motion review: the rare reverse swap (tall result exiting over a short entering card) can overhang the collapsed slot for ~200ms since `.cs-lens-slot` has no overflow clip; and the sub-second run flash path was judged unreachable (running only clears when synthesis arrives, tens of seconds) so no show-delay guard was added.
...
- [x] **Step 1: Per family: grep all declarations, record the effective computed rule, author it once in the owner, delete the rest, re-run `audit:css` + gallery screenshot, commit.** Five commits.
- [x] **Step 2: `--font-mono` audit + retarget, own commit.**
- [x] **Step 3: `theme-and-dark.css` shrink, own commit, dark-mode screenshot pass (the panel's warm paper dark mode must survive).**

**Done (2026-07-21):** Commits `e11cf61`..`919060b` (5 families + font-mono + shrink) plus fix `2a9e13a`. Grep-proof holds: lens selectors declared in exactly one file (research-trail.css, all other partials zero), reproduced independently by the reviewer, who also re-derived two full cascades (the 13-touch `.cs-research-layer-head` composite and the `.cs-source-chip:hover` specificity wrinkle) and confirmed the authored winners match what actually won. `theme-and-dark.css` is down to 6 genuinely dark-scoped rules; the relocated unconditional winners keep their cascade position by source order. The review caught a real Critical the suites could not: the `--font-mono` retarget leaked onto the public site through `packages/ui/tokens.css` (apps/web imports it; the card loading skeleton's live selectors fell through to system monospace since no mono webfont loads there). Fix `2a9e13a`: the five live loading-skeleton sites rewritten to explicit `var(--font-text)` (an identity with pre-retarget rendering, since the alias literally equaled it), seven dead index/browse selectors deleted whole-block after independent re-greps, `.cs-footer-copy`/`.cs-footer-meta` rewritten. Final census: 15 `var(--font-mono)` consumers, all either genuine numeric-ledger uses, dead rules, or extension sites shadowed inert by the body-level override (shadowing mechanics verified, not asserted). Re-review Ready. Minor recorded: the fix commit body's stated mechanism for the footer-copy judgment is wrong (the extension branch returns before the footer renders); the fix itself is correct. Implementer stalled once on infrastructure mid-family-1 and was resumed with verified tree state; no work lost.

### Task 3.5: Phase 2 gate

- [x] `npm run check` green (includes extension tests + `audit:css`).
- [x] Full gallery screenshot set posted for Samay's review; remind him: macOS Reduce Motion OFF. Iterate until approved; record verdicts here.
- [x] Grep-proof: `grep -c "cs-investor-read\|cs-lens-" apps/extension/src/styles/*.css` shows lens rules in exactly one file.
- [x] Firefox build (`npm run build:firefox -w @cold-start/extension`) + `web-ext lint` green (already in check); quick manual sidebar smoke via `npx web-ext run --source-dir apps/extension/dist-firefox`.

**Done (2026-07-21, Phase 2 approved and shipped):** Samay approved the gallery set first pass ("Everything looks good"), no iteration requested; the four queued taste items (slashed-mark weight, posture-after-stamp, empty-side prefix, reverse-swap overhang) ship as-is and remain open to revisit on feedback. Firefox build and web-ext lint green inside check; the interactive sidebar smoke was not separately performed and rides on deployed usage. Final slop sweep clean (DESIGN.md's one structural flag is byte-identical on main, pre-existing; zero em-dashes in UI code). Merged ff to main and pushed; Vercel deploy verified serving; extension dist and dist-firefox rebuilt at the shipped tree.
...
- [x] **Step 1: Failing unit tests:** (a) hover then leave at 50ms never opens (fake timers); (b) hover 120ms opens; (c) focus opens with no delay; (d) docked trigger renders with `data-mode="docked"`; (e) click pins, second click unpins, Escape unpins and refocuses (pointer parity with the existing keyboard pin tests); (f) plain-text triggers stay popover.
- [x] **Step 2: Confirm fail. Step 3: Implement.** Geometry in JS stays minimal: docked top = anchor rect bottom + 6px, recomputed on open and scroll (the panel scrolls; reuse the existing position-tracking approach).
- [x] **Step 4: Pass. Step 5: Commit.**

**Done (2026-07-21):** Commit `516ca2c`. shared-tooltip suite 22/22 (10 existing adapted with timer advances, none weakened; 12 new), full extension 267/267, typecheck, lint, gallery 6/6. The Opus reviewer reproduced the TDD red phase itself by reverting the source (9 fail pre-diff with the exact reported errors; 3 are honest regression guards). Anchor drift corrected against the brief: 5 live call sites, not 8 (Phase 2 retired the three lens tooltips). Four implementer judgment calls all endorsed on review: second click demotes the pin in place (a full close would strobe under the still-hovering pointer), a hover-while-pinned guard (WCAG persistent), same-mode-only crossfade retarget, and 16px per-side dock margins. Also fixed a diff-introduced prop-spread collision that would have silently deleted PeopleLine's "+N more" click handler. Carried into Task 4.2: the pinned dossier still carries `role="tooltip"` while interactive (pre-existing; strict APG wants a role promotion on pin). Noted: docked geometry recomputes on open and scroll but not resize (matches the brief's stated scope); scroll-reposition is code-verified only until Task 4.3's real-geometry coverage.
...
- [x] **Step 1: Failing tests: dossier renders the four blocks in order; email contract assertions ported from the existing suite stay green; "+N more" people expansion animates height (assert the wrapper uses the measured pattern, skipAnimations for jsdom).**
- [x] **Step 2-4: Fail, implement (iterate in the gallery; add a `dossier.json` fixture with a rich person + inferred-email person), pass.**
- [x] **Step 5: Commit.**

**Done (2026-07-22):** Commits `972247c` + fix `ddf2051`. Extension 273/273, gallery 7/7 with the new dossier phase, typecheck, lint clean. Four-block hierarchy on the Phase 2 scale roles; read clamps at 3 lines and unclamps on pin; expanded content scrolls inside the dock cap; provenance folds into the read's citation mark; email contract tests untouched and green; channels one receipt row; "+N more" uses the 0fr/1fr measured pattern with reduced-motion instant. ARIA promotion landed: pin flips `role="tooltip"` to `role="dialog"` with aria-label and reverts on unpin, keyboard flow bit-identical, tested both ways. The Opus review caught the phase's sharpest bug so far: the authored identity rules were dead, shadowed by the generic `.cs-shared-tooltip strong/span` selectors, so the name rendered as the old 11px seal display-face tag; jsdom and the gallery assertions cannot see a cascade, and the first implementer misread its own screenshot. Fix `ddf2051` scoped the generic rules to the plain variant's direct children; the fixer reproduced the broken state via stash (gallery stayed green while visually broken), then verified the fresh pixels: name in dark ink Plex at Claim role. Re-review Spec PASS, Taste Approved; the scoping also freed the pre-existing dead `.cs-dossier-email-address` override. dossier.json is a coherent synthetic company (Wharf Robotics) with person shapes mirroring baseten's, judged better than a Frankenstein clone. Known debt to Task 4.3: two sidepanel-ui e2e assertions stale since 4.1's docked switch (compact-layout and floats-beside-trigger).
...
- [x] **Step 1: Write the occlusion regression RED-FIRST against pre-phase code:** check out the commit before Task 4.1, add the test (hover person row 3, then move pointer to row 1's coordinates, assert row 1's dossier content appears within 500ms), run it, confirm FAIL, record the failure output here, then rebase the test onto the phase branch and confirm PASS. This proves the net catches the actual bug.
- [x] **Step 2: Add geometry assertions:** open dossier bounding box never intersects any `.cs-people-person` bounding box; fly-by (pointer crosses all rows in under 80ms) opens nothing.
- [x] **Step 3: Call-site sweep:** one assertion each for the 8 SharedTooltip consumers (description "(more)", person dossier, "+N more" people chip, lens footer sources overflow, `SourceChips` on a deck layer; the three retired lens call sites assert inline disclosure instead). Reduced-motion pass: dossier opens with no transition, still functional.
- [x] **Step 4: Firefox parity:** build + manual sidebar smoke; pointer events differ subtly, verify open-delay and pin by hand; record here.
- [x] **Step 5: Commit. Phase gate: `npm run check` green.**

**Done (2026-07-22):** Commits `06edc5e` (red-first occlusion regression) + `568b636` (geometry, sweep, stale-test strengthening, Firefox parity). Red proof GENUINE at two independent levels: the implementer ran the checkout dance against `ae2f731` and recorded the occlusion-consistent failure (old dossier at `placement: "above"` grows upward over sibling rows at z-index 80; row 1's content never arrives, tooltip stuck on row 3's person), and the reviewer reproduced the entire dance itself, tracing the mechanism through the old source rather than trusting the transcript. Current-code pass confirmed; sidepanel-ui 61/61. Geometry: 16 pairwise AABB non-intersection checks across all four person rows; fly-by crosses all rows under 80ms and asserts nothing opens after a settle window beyond the 90ms gate. Call-site sweep covers the real 5-consumer inventory (plan's 8 predates Phase 2 retiring three lens tooltips) plus inline-disclosure assertions for the retired sites and a functional reduced-motion pass. The two assertions stale since 4.1's docked switch were strengthened to the docked contract (verified they cannot pass under old popover geometry: the width floor exceeds the popover cap). Firefox build and web-ext lint green; the by-hand pointer smoke (open delay, pin) is explicitly deferred to human review, not faked. Low note for CI watch: the fly-by wall-clock budget is the file's only raw timing assertion; 8 local reruns no flake. Side item cleared at the gate: a new upstream sharp/libvips advisory (GHSA-f88m-g3jw-g9cj) was overridden to sharp 0.35.3 in `5de0a2d`, web build verified, no allowlisting.

**Done (2026-07-22, Phase 3 shipped):** Whole-branch Opus review found one phase-introduced blocker the task reviews missed: collapsed overflow person rows stayed Tab-reachable at zero height, opening a dossier from an invisible trigger. Fixed in `15f34c9` with the codebase's own visibility-toggle pattern on both collapse frames, proven red-first in a real browser, plus the pinned-state geometry sweep that closed the plan's flagged coverage gap. Re-review Ready to merge YES; check green twice at the tip. Every seam held: pin/role promotion cannot desync, the identity-specificity scoping reaches exactly the plain consumers, the email contract is untouched git-level, no contract bump needed. Ship-with polish recorded, not blocking: a focus-opened unpinned dossier has no in-place Escape (clears on tab-away; the pinned dialog is fully WCAG 1.4.13 compliant), and docked geometry recomputes on open and scroll but not resize. The Firefox by-hand pointer smoke (open delay, pin) remains the one human item, deferred honestly. Merged ff to main and pushed; extension rebuilt in dist and dist-firefox.
...
- [x] **Step 1: Failing tests:** pipeline unit: synthesize unit callable without verify; verify unit callable with a stored synthesis draft; gate-withheld path emits neither event. Helper test: step sequencing preserves the existing trace shape (`steps.synthesize-card`, `steps.verify-synthesis` durations still recorded).
- [x] **Step 2-4: Fail, implement, pass.** Confirm `analysisReadyMs` semantics unchanged (stamped at card-saved with synthesis, as today).
- [x] **Step 5: Deploy note in the commit body: new step ids alter durable-execution identity for in-flight runs; deploy in a quiet window (US evening), then `npm run repair:stuck-runs` if anything strands. Commit.**

**Done (2026-07-22):** Commits `db19253` (split) + `396b007` (review comment fixes). Pipeline 80/80, web 172/172, extension 273/273 unchanged, typecheck, lint, knip clean. New step ids exactly `synthesize-card` and `verify-synthesis`; existing ids byte-identical; the three events memoize through `step.run("event-...")` so replays never re-emit; a verify retry provably replays the memoized synthesis draft instead of re-paying the ~40s Sonnet call. Zero `packages/llm` diff. The Opus review walked the one named seam concretely and ruled it SAFE: the gate decision recomputed in the plain body is pure over the memoized card, and the withheld `at` timestamp persists exactly once through the single memoized `upsert-card` write, so replays cannot persist divergent values; the route's free pre-check compares evidence counts, never `at`, and is untouched. `sidepanel-network.ts` holds no exhaustive event switch (Set membership over four card-ready literals), so the additive vocabulary needs no contract bump. Honest gap on record: the web test harness executes `step.run` bodies directly and never memoizes, so replay safety rests on the reviewed reasoning plus Inngest's contract, not a test. `verifiedSynthesisForCard` is now a reference-only combined path (production passes no synthesis deps); marked in code. Deploy rider: quiet window plus a `repair:stuck-runs` sweep, per the commit body.
...
- [x] **Step 1: Failing routing tests for the three flag values x fresh/stale.**
- [x] **Step 2-3: Fail, implement.**
- [x] **Step 4: Shadow comparison:** 20+ analysis runs across golden-set companies (use `npm run qa:generation` fixtures), 10+ in `full`, 10+ in `skip-fresh`, matched slugs. Compare: verifier-surviving claim count, `hasInvestorUsableProfile`, citation counts, and eyeball three read pairs blind. Record the table as a Finding here.
- [x] **Step 5: Promotion decision with Samay on the evidence (flip the Vercel env or hold). The flag stays either way as the rollback. Commit.**

**Done (2026-07-22, promoted):** Samay approved on the combined 15-pair evidence (claims 31 to 31 net zero with per-pair variance traced to synthesis non-determinism; citations down 5.2%, no gate impact; AgentCash eliminated; wall down 10.1s median). Flip executed with deploy discipline: zero in-flight runs verified, `ANALYSIS_SOURCE_REFRESH=skip-fresh` set in Vercel production, SPEC.md and the cost-assumptions correction (measured ~10s, not ~30s) riding the deploy commit, `repair:stuck-runs` swept clean after. Rollback is removing the env var. The two-week measurement watches citation counts as the tripwire.

**Finding (2026-07-22, shadow table):** 42 runs total ($8.78 against the $12 cap; wallet $7.46 to $5.21), 10 matched slugs. The rig initially diverged from prod (no verifier routing in `.env.local`, so verify ran on Sonnet and truncated its plain-JSON response at the hardcoded 2000 max_tokens on rich slugs); caught mid-batch, fixed with prod-parity DeepSeek routing, mixed-routing pairs excluded. On prod-parity routing, n=5 clean pairs: verifier-surviving claims 14 to 16 (zero drops, two gains, three ties), quality gates 20/20 (though `analysisBlockedReason` never reads synthesis, so not discriminating), citations flat to up, AgentCash $1.29 to $0.00 (100% of gated spend eliminated), Anthropic flat, wall faster in 8 of 9 comparable pairs with ~8.5s median savings (the docs' ~30s estimate was optimistic; the probe fetch overlaps Direct Exa, so only the serial tail is saved; provider-cost-assumptions to be corrected at promotion). Blind read of three pairs: no quality difference. Legacy Sonnet-verify pairs showed drops but measure a routing prod does not run. Full table and appendix: `.superpowers/sdd/task-5.3-shadow-report.md`. Promotion evidence is clean but thin (n=5 vs the 20-run bar); the decision goes to Samay with the run-more-pairs option costed (~$5, ~10-15 pairs). Side observations recorded for the gate: the all-claims-dropped throw surfaced twice as a run failure (pre-existing surface Phase 1 did not restyle), and one transient Zod null-field synthesis draft error resolved on retry.
...
- [x] **Step 1: Failing fixture-driven tests:** replaying `running-events.json` advances stages in order; an event gap produces no blank state (previous stage holds, receipt-face elapsed keeps counting); `verify.complete` claim count renders; the final event hands off to the read without unmount flash (exit-state rule).
- [x] **Step 2-4: Fail, implement against the gallery (add the wait phases to the gallery spec), pass. Gallery screenshots + a screen recording for review (motion cannot be reviewed in stills).**
- [x] **Step 5: Commit.**

**Done (2026-07-22):** Commits `d0ae364` + review fixes `dd66745`. Extension 293/293 incl. audit:css, gallery 7/7 with running-events re-screenshotted, typecheck, lint, knip, full monorepo test green. Event truth confirmed by the Opus reviewer against production emission code: stages map only to real event types, the verify stamps bind to `metadata.claimCount` from the real 5.2 payload with the prose regex as documented fallback, and the fixture was corrected byte-level to production copy with a regression test locking it. Gap and withheld paths walked: highest-reached-index holds the stage through the real ~25s gap, and a gate-withheld run marks Read/Verify skipped rather than falsely done. Resume plumbing closed in BOTH controllers (the second was undocumented by the Task 3.1 report) and bootstrap now seeds from `analysisStatus.events`, delivering backscroll-on-reconnect per the reference library. "Reusing filed evidence" binds to 5.3's `analysisSourceRefresh` metadata and degrades on old runs. One signature loop holds; every animated element has a reduced-motion story ending in the shared breathe pattern; stamps are marks, never claim text. Review's two lows fixed same-day: retries start with an empty event list (no one-tick fully-advanced flash), and the live region announces stage transitions only instead of ticking every second. Deferred to the review session: live motion feel (stamp stagger, breath, WebGL mesh drift), stills cannot carry it.
...
- [x] Immediately post-deploy: one live analysis run traced end-to-end (`npm run trace:generation` locally + a real deployed run); confirm events flow, wait surface stages, staged entrance lands, `repair:stuck-runs` finds nothing. See the 2026-07-22 uniqlo entry in `docs/qa/analysis-run-observations.md` (all new step events fired in production for the first time; logged there as this task's live deployed trace).
