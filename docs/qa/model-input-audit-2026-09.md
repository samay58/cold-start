...
4. Person reads: evidence is text about the named person, each person is read once, and suppression reasons reach the trace. Open: Ivan Zhao has no person-read evidence once his Apollo record is not read as JSON. The title-only rule is not limited to person reads: every stage reads a people or organization record as its title, since the record has no page text. The September 24 review kept it that way. On the 12 source sets such records are 9 of 1,021 rows, and their fields already reach the card as cited provider facts, which every stage reads. The same review gave person reads whole sentences instead of windows cut at the next name (`docs/qa/review-2026-09-24-evidence-remediation.md`, findings 2 and 19).
