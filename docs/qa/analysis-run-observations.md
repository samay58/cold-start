...
3. **skip-fresh promotion: DONE (2026-07-22, promoted).** Evidence: 15 prod-parity pairs. Combined: claims 31 to 31 net zero (4 drops, 6 ties, 5 increases; every drop traced to synthesis non-determinism, not a mechanism), citations down 5.2% (mechanically expected; zero gate impact), AgentCash 100% eliminated with no exceptions, wall down 10.1s median (15.3%) in 10 of 12 comparable pairs. Decision is Samay's; the flip is a quiet-window operation (adds a conditional step to in-flight runs), and the env var is the instant rollback. If promoted, the two-week measurement watches citation counts.
