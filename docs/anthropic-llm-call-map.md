...
5. **Synthesis verification**: LLM-as-judge over the synthesis claims against citation snippets, returning supported/contradicted/unsupported per claim. `applyVerifierResults` (`verifier.ts:56`) then drops everything not exactly-once-supported; the pipeline never restores drops. Its plain-JSON response is capped by a hardcoded `max_tokens: 2000` (`verifier.ts:116`); rerouting `LLM_VERIFIER_MODEL` to a more verbose model reintroduces silent truncation, not a clean failure (six of eight shadow-comparison failures hit exactly this ceiling under a local Sonnet-routed rig before prod-parity DeepSeek routing was restored; `docs/qa/analysis-run-observations.md` attack item 4). Confirm response fit before shipping any `LLM_VERIFIER_MODEL` change.
