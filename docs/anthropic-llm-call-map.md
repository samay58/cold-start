...
| 10 | `synthesizeHowItWins` | `how-it-wins.ts` | `how_it_wins` | `how-it-wins-reason`, `how-it-wins-edit`, `how-it-wins-editor`, `how-it-wins-fit` | 16000 (8000 on the editor pass) | provider default | none (JSON text) | live |
...
Function 10 is the exception to the single-call shape: it parses its own JSON and runs its own corrective re-ask, and its writer passes go through `withProviderFallback` on the `how_it_wins` stage.
...
10. **How it wins** (added 2026-08-19, live, the Lens edge read): names the two to four ways this company actually wins, drawn from a fixed vocabulary of 80 strategies in 13 groups (`packages/core/src/how-it-wins.ts`), plus the one pair of them that compounds. It is four passes over one card, not one call. Pass 1 (`how-it-wins-reason`) reasons in prose under the writing standard. Pass 2 (`how-it-wins-edit`) fits that reasoning to the panel's slots as JSON. Pass 3 (`how-it-wins-editor`) is a second model reading the draft as a hostile editor; it runs on `LLM_HOW_IT_WINS_EDITOR_MODEL` (default `deepseek/deepseek-v4-pro`) and is optional by design, so any failure at all carries the pass-2 draft forward unchanged. Pass 4 (`how-it-wins-fit`) trims the result to the surface, with one corrective re-ask when the draft does not parse or breaks the style rules. All four record the stage `how_it_wins`; the writer passes route through `LLM_HOW_IT_WINS_MODEL`, then the synthesis chain, with `LLM_HOW_IT_WINS_FALLBACK_MODEL` for alternate-provider continuity. The thin-file gate (`howItWinsThinFileReason`, the same gate the emphasis read uses) runs in code before any of it, so a thin card pays for none of the four passes.
...
| `analysis`, fresh | 3 to 13 (1 `extract_full` + 0-5 `extract_block` + 1 `synthesis` + 0-1 `emphasis_read` + 0-4 `how_it_wins` writer passes + 1 `verify`) |
| `analysis`, reusing stored profile | 2 to 7 (1 `synthesis` + 0-1 `emphasis_read` + 0-4 `how_it_wins` writer passes + 1 `verify`) |
...
`how_it_wins` is 0 when the card is thin-filed, `HOW_IT_WINS_ENABLED=false`, or the run predates 2026-08-19. Otherwise the table counts the writer passes only: 3 in the normal case (reason, edit, fit) and 4 when the fit pass needs its one corrective re-ask. The hostile editor between edit and fit runs on `LLM_HOW_IT_WINS_EDITOR_MODEL` (default `deepseek/deepseek-v4-pro`), so it bills DeepSeek rather than Anthropic and is not an Anthropic call at all. On top of that, the empty-text ladder can add up to two more calls per writer pass in the worst case, one at the higher token ceiling and one with thinking disabled, so a pathological run reaches 12 Anthropic calls for this stage.
