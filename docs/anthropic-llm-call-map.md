...
Every place Cold Start calls Anthropic's API or any LLM. First mapped at commit `66362f0` (2026-06-11); updated the same day for per-stage provider routing, on 2026-07-20 for person reads and OpenRouter, on 2026-07-23 for the verifier `max_tokens` cap raise, on 2026-07-27 for the expanded-description stage, on 2026-08-11 for alternate-provider continuity, on 2026-08-12 for the emphasis-read stage, and on 2026-08-19 for the how-it-wins stage. Line numbers drift; the stable identifiers are the trace `stage`, `label`, and `provider` values, which are recorded on every call.

The one-sentence summary: every production LLM call goes through a single provider-aware chokepoint, `createTracedAnthropicMessage` in `packages/llm/src/anthropic.ts` (`client.messages.create` at line 134 for Anthropic models; provider-prefixed model strings dispatch to the OpenAI-compat adapter). There are exactly nine call functions wrapping that chokepoint, eight of them live, one dormant. Anthropic is the default provider for every stage; DeepSeek or any OpenAI-compatible host can be flipped in per stage via env. Outside production there is one more direct Anthropic caller (a cache diagnostic script) and the provider-matrix replay harness which calls the same stage functions over frozen fixtures.
...
| `emphasis_read` | `LLM_EMPHASIS_READ_MODEL` | `LLM_SYNTHESIS_MODEL`, then `ANTHROPIC_SYNTHESIS_MODEL` | `ANTHROPIC_MODEL` |
| `how_it_wins` | `LLM_HOW_IT_WINS_MODEL` | `LLM_SYNTHESIS_MODEL`, then `ANTHROPIC_SYNTHESIS_MODEL` | `ANTHROPIC_MODEL` |
...
## The nine call functions in packages/llm
...
| 9 | `synthesizeEmphasisRead` | `emphasis-read.ts:111` | `emphasis_read` | `emphasis-read` | 1200 | 0.2 | `emit_emphasis_read` | live |
| 10 | `synthesizeHowItWins` | `how-it-wins.ts` | `how_it_wins` | `how-it-wins:monolith`, `how-it-wins:critic`, optional `how-it-wins:adjudication`, then `how-it-wins-frozen-writer` | 50000/12000 on judge stages; 16000 on the writer | 0 on judge; provider default on writer | `emit_how_it_wins_judgment` then JSON text | live |

Functions 2 through 9 wrap their call-and-parse in `withSchemaRetry`, so a non-Anthropic model gets one re-ask on unparseable output; on Anthropic models the wrapper is a no-op. Synthesis, verification, research sections, person reads, expanded descriptions, and the emphasis read also use `withProviderFallback`. Their primary model remains unchanged. A fallback runs once on the configured alternate provider only after a provider-availability failure.

Function 10 is judge-then-writer, and since 2026-08-25 it runs outside the analysis run (the `how-it-wins-read` paragraph below). `judgeHowItWinsForAnalysis` runs a monolith all-80 verdict (`scopes: []`) through `createHowItWinsJudge`, with `LLM_HOW_IT_WINS_JUDGE_MODEL` as the judge and the editor model as the critic (different providers, checked at construction); a critic or adjudication failure keeps the global verdict and is recorded under `refinement` on the judgment. `synthesizeHowItWins` then renders that frozen verdict in one writer call (`how-it-wins-frozen-writer`), skips the hostile editor, and stamps `editorSkipped`. The four-pass path remains for callers that omit `judgment` (the corpus script). Display running is capped at six strategies with a floor of one, in question at twelve; the full verdict stays on `trace.howItWins.judgment` and in `how_it_wins_judgments`.
...
5. **Synthesis verification**: LLM-as-judge over the synthesis claims against citation snippets, returning supported/contradicted/unsupported per claim. `applyVerifierResults` (`verifier.ts:56`) then drops everything not exactly-once-supported; the pipeline never restores drops. Its plain-JSON response is capped by a hardcoded `max_tokens: 8192` (`verifier.ts:116`, raised from 2000: six of eight shadow-comparison failures hit the old ceiling under a local Sonnet-routed rig before prod-parity DeepSeek routing was restored; `docs/qa/analysis-run-observations.md` attack item 4). The cap is a ceiling against truncation, not a spend target; prod DeepSeek verify has never hit either ceiling. Confirm response fit before shipping any `LLM_VERIFIER_MODEL` change that routes to a more verbose model.
...
8. **Expanded description** (added 2026-07-27, live): writes the card's long-form `expandedDescription` (three short paragraphs, 60-280 words total) from cited card facts and sources, inside the block-enrichment worker. A draft leaning on the brochure phrases in `expanded-description.ts`'s `BANNED_PHRASES` list is suppressed rather than repaired; the card falls back to its short description and the reason is recorded in the trace step.
9. **Emphasis read** (added 2026-08-12, live, the Pay attention to row of the Investor Lens): from source digests over the card's citations plus the founder-voice evidence just fetched, writes what the company and its founders are loud about (`loud`), what the filed record never shows (`quiet`, always opening "Nothing filed shows"), the smallest cited inference that asymmetry supports (`read`), and what would break it (`wouldChangeIf`). The thin-file gate (`emphasisThinFileReason`, `packages/core/src/emphasis-read.ts`) runs in code before this call, so a thin card never reaches it; the model itself may still emit `nothing_notable` when no specific cited asymmetry exists.

10. **How it wins** (added 2026-08-19, judge-then-writer as of 2026-08-24, the Lens edge read): names the ways this company actually wins, drawn from a fixed vocabulary of 80 strategies (`packages/core/src/how-it-wins.ts`). Analysis now judges first (`judgeHowItWinsForAnalysis`, monolith, `scopes: []`) and then renders the frozen verdict (`synthesizeHowItWins` with `judgment`). The writer may not add, drop, swap, or reorder labels. The hostile editor is skipped on this path. Canonical meanings come from core. Display running is capped at six strategies with a floor of one (`HOW_IT_WINS_DISPLAY_RUNNING_MAX`); the uncapped all-80 audit stays on the private generation-run trace and in `how_it_wins_judgments`. The thin-file gate (`howItWinsThinFileReason`) still runs in code before any of it. The four-pass writer remains for the corpus script when no frozen verdict is supplied.

## Where production calls happen: apps/web/src/inngest/

The worker registration and step boundaries live in `apps/web/src/inngest/functions.ts`. `generateCardFunction` builds the client and resolves the six live stage models once per run via `modelForStage` (lines 386-391, including `sectionModel` for `research_section` and `emphasisModel` for `emphasis_read`), then injects closures into the pipeline. The section branch delegates the pure evidence-to-section work to `apps/web/src/inngest/research-section-generation.ts`, but the Inngest `generate-section` step, trace merge, and event writes stay in `functions.ts`. `packages/pipeline/src/generate-card.ts` never imports any LLM client itself; it sequences the injected functions.
...
- Synthesize and verify (Phase 4 Task 5.2, split from one atomic step into two): only run when `mode === "analysis"` and the pipeline's `evaluateSynthesisGate` (`generate-card.ts`) does not block on thin evidence; a blocked run calls neither. When it proceeds, `synthesizeCard` runs inside `step.run("synthesize-card")` via the pure `synthesizeCardStepBody` helper (`generation-helpers.ts`), which wraps the pipeline's `synthesizeCardDraft`; its output (the synthesis draft plus claim count) is a separately memoized Inngest step result. `verifySynthesis` then runs inside its own `step.run("verify-synthesis")` via `verifySynthesisStepBody`, wrapping `verifyCardSynthesisDraft`, over all synthesis claims from that stored draft. Because the two calls are separate steps, a verify-only retry (transient failure, deploy mid-run) replays the memoized synthesis draft instead of re-paying for the Sonnet synthesis call. Progress events `synthesis.started`, `verify.started`, and `verify.complete` bracket the two calls; the gate-blocked path emits none of them. `verifiedSynthesisForCard` (`generate-card.ts`) still exists as a thin wrapper composing the two split units, used only by the combined, non-split call path that pipeline-level tests exercise directly.
- Emphasis read (added 2026-08-12, the Pay attention to row of the Investor Lens): runs between `synthesize-card` and `verify-synthesis`, gated by `EMPHASIS_READ_ENABLED` (default on) and a code-only thin-file check, `emphasisThinFileReason` (`packages/core/src/emphasis-read.ts`), that runs before either step so a thin card pays for neither. When it proceeds, `step.run("fetch-founder-voice")` calls `fetchFounderVoiceStepBody` (`apps/web/src/inngest/emphasis-read.ts`), which runs `fetchFounderVoiceEvidence` (`packages/providers/src/founder-voice/`) across the five founder-voice lanes and turns hits into `fv`-prefixed citations, added to the working card additively. `step.run("emphasis-read")` then calls `emphasisReadStepBody`, wrapping `synthesizeEmphasisRead` (function 9 above) over source digests built from those citations. The emphasis claims ride the existing `verify-synthesis` call rather than paying for a second verifier round trip; `verified.emphasisRead` is read independently of whether any synthesis claim survived. Progress events `emphasis.started` and `emphasis.complete` bracket the pair; a thin-file or disabled run emits neither and marks both steps skipped in the trace. A post-verify prune caps cross-run `fv` citation accumulation once the final read is known.
- How it wins (added 2026-08-19, moved out of the analysis run on 2026-08-25): the analysis run no longer runs a `how-it-wins` step of its own. After verification it runs the code-only thin-file check (`howItWinsThinFileReason`), files `{status: "thin_file"}` on the card when it fails, and otherwise marks the trace `howItWins.status: "deferred"`, emits `how-it-wins.started`, and sends `card/how-it-wins.requested` after the store step. The `how-it-wins-read` function (`apps/web/src/inngest/how-it-wins-function.ts`, steps `how-it-wins-load`, `-judge`, `-write`, `-verify`, `-store`) pays for the judge, the writer, and its own verifier call, stores the judgment in `how_it_wins_judgments`, writes the read onto the card without extending the synthesis TTL, and stamps the parent run's trace (`howItWins.status` becomes `read`, `nothing_stands_out`, `failed`, or `stale` when the card's evidence moved under it) before emitting `how-it-wins.complete` with `metadata.status` onto the parent trail. The verifier degrades the read to `nothing_stands_out` only when no running strategy survives. A thin-file run emits no start event, marks the step skipped, and closes the trail itself with `how-it-wins.complete` carrying `status: "thin_file"`, since no background function will; a disabled run marks the step skipped and emits neither event.
...
| `analysis`, fresh | 3 to 12 (1 `extract_full` + 0-5 `extract_block` + 1 `synthesis` + 0-1 `emphasis_read` + 0-2 `how_it_wins` judge calls + 0-1 optional adjudication + 0-2 frozen writer calls + 1 `verify`) |
| `analysis`, reusing stored profile | 2 to 8 (1 `synthesis` + 0-1 `emphasis_read` + 0-2 `how_it_wins` judge calls + 0-1 optional adjudication + 0-2 frozen writer calls + 1 `verify`) |
...
`emphasis_read` is 0 when the card is thin-filed, `EMPHASIS_READ_ENABLED=false`, or the run predates 2026-08-12; otherwise 1, regardless of whether the model emits `read` or `nothing_notable`.

`how_it_wins` calls no longer run inside the analysis run, but they still land on its ledger: a paid judgment's calls (`how-it-wins:global_judge`, `:critic`, `:adjudication`) are copied onto the parent run's `trace.llm.calls` by `howItWinsJudgeLlmCalls`, so `cost_usd` counts the whole read; a cached judgment adds nothing. The writer and its verifier call reach the same ledger through the step telemetry sink. Since 2026-08-25 the judge and writer run in the separate `how-it-wins-read` Inngest function (`apps/web/src/inngest/how-it-wins-function.ts`), which the analysis run dispatches (`card/how-it-wins.requested`) only after it stores fresh synthesis. That function pays nothing when the card is thin-filed, `HOW_IT_WINS_ENABLED=false`, or a cached judgment already exists in `how_it_wins_judgments` for the same evidence-packet, prompt, and vocabulary hashes; otherwise it pays for the monolith judge (`how-it-wins:monolith` on `LLM_HOW_IT_WINS_JUDGE_MODEL`, falling back to the writer model `LLM_HOW_IT_WINS_MODEL`) plus a critic (`how-it-wins:critic` on `LLM_HOW_IT_WINS_EDITOR_MODEL`, default `deepseek/deepseek-v4-pro`; the critic must be a different provider from the judge). Material critic findings add one adjudication call. The frozen writer is then one call, two if it needs its one corrective re-ask. The judge call streams (`client.messages.stream` on Anthropic, the OpenAI-compat call otherwise): a non-streaming Anthropic call throws client-side above 21333 `max_tokens`, and measured judge output runs 24k-33k tokens, which is why production never produced a read before the 2026-08-25 repair. The four-pass labels (`how-it-wins-reason` / `edit` / `editor` / `fit`) remain only on the corpus path that omits a frozen verdict.

The judge's cached prefix (rules, vocabulary, tool schema; byte-identical across companies because the evidence handle universe is fixed at 200) is written to the 1h Anthropic cache on every cold call. A 1h write costs 2x the input price, and it only pays back when a second call lands inside the hour: the adjudication patch, a repair re-ask, or another company's judge. Measured 2026-08-25, a warm judge call cost $0.39 against $0.81 cold. At one analysis an hour or less, most judge calls are cold and the write is pure overhead, so this is the cost model to revisit first if traffic stays sparse (`ANTHROPIC_CACHE_TTL=5m` drops the write from 2x to 1.25x without a deploy, at the price of a miss on any second call more than five minutes later).
...
| `LLM_EXTRACT_MODEL`, `LLM_BLOCK_MODEL`, `LLM_VERIFIER_MODEL`, `LLM_SYNTHESIS_MODEL`, `LLM_RESEARCH_SECTION_MODEL`, `LLM_PERSON_READ_MODEL`, `LLM_EXPANDED_DESCRIPTION_MODEL`, `LLM_EMPHASIS_READ_MODEL`, `LLM_HOW_IT_WINS_MODEL`, `LLM_RESEARCH_PLAN_MODEL` | per-stage provider routing; accept `provider/model` strings; checked before the ANTHROPIC_* equivalents |
| `LLM_FALLBACK_MODEL`, `LLM_SYNTHESIS_FALLBACK_MODEL`, `LLM_VERIFIER_FALLBACK_MODEL`, `LLM_RESEARCH_SECTION_FALLBACK_MODEL`, `LLM_PERSON_READ_FALLBACK_MODEL`, `LLM_EXPANDED_DESCRIPTION_FALLBACK_MODEL`, `LLM_EMPHASIS_READ_FALLBACK_MODEL`, `LLM_HOW_IT_WINS_FALLBACK_MODEL` | alternate-provider continuity for judgment stages; stage-specific values win; used only for provider unavailability and ignored when they resolve to the primary provider |
...
| `HOW_IT_WINS_ENABLED` | `false` disables the background `how-it-wins-read` Inngest function the analysis run dispatches after storing synthesis (default on; production on since the 2026-08-25 repair deploy; a change applies on the next deploy) |
...
- `withSchemaRetry` stays a no-op for Anthropic models. `withProviderFallback` may call a different provider after an availability failure, but must never retry the failed Anthropic request or hide authentication and contract failures.
