...
Function 10 is now judge-then-writer. `judgeHowItWinsForAnalysis` runs a monolith all-80 verdict (`scopes: []`) through `createHowItWinsJudge`, with the writer model as the strong judge and the existing editor model as the critic (they must be different providers). `synthesizeHowItWins` then renders that frozen verdict in one writer call (`how-it-wins-frozen-writer`), skips the hostile editor, and stamps `editorSkipped`. The four-pass path remains for callers that omit `judgment` (the corpus script). Display running is collapsed to at most four strategies; the full verdict stays on `trace.howItWins.judgment`.
...
10. **How it wins** (added 2026-08-19, judge-then-writer as of 2026-08-24, the Lens edge read): names the ways this company actually wins, drawn from a fixed vocabulary of 80 strategies (`packages/core/src/how-it-wins.ts`). Analysis now judges first (`judgeHowItWinsForAnalysis`, monolith, `scopes: []`) and then renders the frozen verdict (`synthesizeHowItWins` with `judgment`). The writer may not add, drop, swap, or reorder labels. The hostile editor is skipped on this path. Canonical meanings come from core. Display running is collapsed to two-to-four strategies; the uncapped all-80 audit stays on the private generation-run trace. The thin-file gate (`howItWinsThinFileReason`) still runs in code before any of it. The four-pass writer remains for the corpus script when no frozen verdict is supplied.
...
- How it wins (added 2026-08-19, the Lens edge read): `step.run("how-it-wins")` calls `howItWinsStepBody` (`apps/web/src/inngest/how-it-wins.ts`), gated by `HOW_IT_WINS_ENABLED` (default on) and the same code-only thin-file check. It runs concurrently with the founder-voice fetch and the emphasis-read step, because the monolith judge plus frozen writer cost their own model time and nothing it reads depends on the founder-voice citations. The two closures are joined with `Promise.allSettled` and the rejection is rethrown by hand, not `Promise.all`: a failure in either still fails the run, but waiting for both to settle means the loser can never still be calling `step.run` or writing the trace while the failure path is unwinding. The join also stamps the failing closure's stage onto the failure trace, so a how-it-wins failure is not filed under `emphasis-read`. It reads a snapshot of the card taken right after the synthesis draft, so the emphasis closure's citation append never changes the card under it. Its running notes and its pair note ride the same `verify-synthesis` call as the emphasis claims, appended after them; the verifier's verdicts feed `applyHowItWinsVerification`, which kills the pair with either leg and degrades the whole read to `nothing_stands_out` below two surviving strategies. Progress events `how-it-wins.started` and `how-it-wins.complete` bracket it; a thin-file or disabled run emits no start event and marks the step skipped.
