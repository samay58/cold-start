...
| `analysis`, fresh | 3 to 14 (1 `extract_full` + 0-5 `extract_block` + 1 `synthesis` + 0-1 `emphasis_read` + 0-5 `how_it_wins` + 1 `verify`) |
| `analysis`, reusing stored profile | 2 to 8 (1 `synthesis` + 0-1 `emphasis_read` + 0-5 `how_it_wins` + 1 `verify`) |
...
`how_it_wins` is 0 when the card is thin-filed, `HOW_IT_WINS_ENABLED=false`, or the run predates 2026-08-19. Otherwise it is 3 when the hostile editor pass is skipped and the fit pass parses on the first try, 4 in the normal case, and 5 when the fit pass needs its one corrective re-ask.
