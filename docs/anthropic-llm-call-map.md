...
`how_it_wins` is 0 when the card is thin-filed, `HOW_IT_WINS_ENABLED=false`, or the run predates 2026-08-19. Otherwise analysis pays for the monolith judge (`how-it-wins:monolith` on the writer-model chain) plus a critic (`how-it-wins:critic` on `LLM_HOW_IT_WINS_EDITOR_MODEL`, default `deepseek/deepseek-v4-pro`; the critic must be a different provider from the judge). Material critic findings add one adjudication call. The frozen writer is then one call, two if it needs its one corrective re-ask. The four-pass labels (`how-it-wins-reason` / `edit` / `editor` / `fit`) remain only on the corpus path that omits a frozen verdict.
...
| `HOW_IT_WINS_ENABLED` | `false` disables the `how_it_wins` stage in the analysis path (default on) |
