...
Every place Cold Start calls Anthropic's API or any LLM. First mapped at commit `66362f0` (2026-06-11); updated the same day for per-stage provider routing, on 2026-07-20 for person reads and OpenRouter, on 2026-07-23 for the verifier `max_tokens` cap raise, on 2026-07-27 for the expanded-description stage, on 2026-08-11 for alternate-provider continuity, and on 2026-08-12 for the emphasis-read stage. Line numbers drift; the stable identifiers are the trace `stage`, `label`, and `provider` values, which are recorded on every call.
...
| `analysis`, fresh | 3 to 9 (1 `extract_full` + 0-5 `extract_block` + 1 `synthesis` + 0-1 `emphasis_read` + 1 `verify`) |
| `analysis`, reusing stored profile | 2 to 3 (1 `synthesis` + 0-1 `emphasis_read` + 1 `verify`) |
...
| `LLM_EXTRACT_MODEL`, `LLM_BLOCK_MODEL`, `LLM_VERIFIER_MODEL`, `LLM_SYNTHESIS_MODEL`, `LLM_RESEARCH_SECTION_MODEL`, `LLM_PERSON_READ_MODEL`, `LLM_EXPANDED_DESCRIPTION_MODEL`, `LLM_EMPHASIS_READ_MODEL`, `LLM_RESEARCH_PLAN_MODEL` | per-stage provider routing; accept `provider/model` strings; checked before the ANTHROPIC_* equivalents |
| `LLM_FALLBACK_MODEL`, `LLM_SYNTHESIS_FALLBACK_MODEL`, `LLM_VERIFIER_FALLBACK_MODEL`, `LLM_RESEARCH_SECTION_FALLBACK_MODEL`, `LLM_PERSON_READ_FALLBACK_MODEL`, `LLM_EXPANDED_DESCRIPTION_FALLBACK_MODEL`, `LLM_EMPHASIS_READ_FALLBACK_MODEL` | alternate-provider continuity for judgment stages; stage-specific values win; used only for provider unavailability and ignored when they resolve to the primary provider |
