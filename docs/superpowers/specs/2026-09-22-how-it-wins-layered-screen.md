...
- A scoped verdict is filed under the screen's identity (`HOW_IT_WINS_SCREEN_IDENTITY`: version, Jev model and thresholds), not under the scope it produced. Jev's answers vary between calls, so a key built from the scope missed on every re-file and re-paid the judge for unchanged evidence (review of September 22). A re-file now checks the table before screening and replays without asking Jev. A version, model or threshold change still retires every scoped verdict. Every unscoped hash is byte-identical to before, which a test pins, so no filed verdict was invalidated. Scoped and full verdicts never replay for each other.
- The citation check asks Jev, once per current ruling and in parallel, whether the cited evidence shows the stated mechanism. A score below 0.3 becomes a material critic finding of kind `evidence`, and the existing adjudication settles it once. It runs beside the critic call. With refinement off it is skipped, since only adjudication reads its flags. A failed check is a note, never a failed run.
