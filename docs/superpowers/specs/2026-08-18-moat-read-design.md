...
- Open. Whether the lilac seal appears on this object at all. The mock has the pinned state twice, with the seal and with ink only. Decided by eye, by Samay.
- Under discussion, lower confidence. Fold What must be true and What could break into one row using the existing holds/breaks marks (each is empty on 22 to 24% of cards, both on 12%; their content reappears in the questions' tests line). This reverses the July 2026 split and needs Samay's explicit call.
...
- Pin: a click while on a target pins it. Scrubbing stops following the pointer; the mark takes the accent (seal or ink, per the open decision); a "pinned" receipt appears in the note's corner. A second click or Escape releases with a 120ms fade.
