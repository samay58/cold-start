...
- Finish the evidence and judgment remediation. Tasks 0 to 3 and the judge thinking fix are deployed (see Recently shipped). Task 4's accepted-proof wording is committed on local main (`441aa70`) and not pushed, so it has not deployed. Next: Task 5, the fair judge comparison, whose budget (over $10) is Samay's; Task 6, a customer-evidence search pilot capped at $5; then Task 7. Still Samay's: whether to backfill snippets on the roughly 400 existing profiles (they read as text already, because the card schema cleans old raw quotes on read, but rows stored without page text stay title-only until re-filed). Each card's first re-file after this deploy misses its memoized How it wins judgment once, because the evidence packet hash changed. Numbers: `eval/curation/remediation-2026-09/README.md`. See [the plan](superpowers/plans/2026-09-23-evidence-and-judgment-remediation.md).
