...
- Run the evidence and judgment remediation, which comes before choosing the judge. Tasks 0 and 1 are done; the audit (`docs/qa/model-input-audit-2026-09.md`) found eight paid searches that never ask for page text, and Samay approved every finding on September 23. Next is Task 2. The work: give the models the page text instead of search metadata, stop stating normal private-company absences, accept the proof the rubric already allows, then rerun the judge comparison fairly. See [the plan](superpowers/plans/2026-09-23-evidence-and-judgment-remediation.md).
