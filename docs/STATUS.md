...
- Run the evidence and judgment remediation, which comes before choosing the judge. Tasks 0 to 2C are built on local main (not pushed, not deployed): every Exa search asks for page text, every model reads page text instead of provider JSON, person reads read about the right person, own-site pages are labeled as the company, publish dates reach the judge, and the keyword filter on verified claims is gone. Waiting on Samay: (1) decision E, because page-lead snippets cut verifier-kept claims from about 44 to about 24 of 74 while keeping the extraction model's note restores 41 to 43; (2) two Opus 5 judge timeouts at 240 s on rebuilt cards; (3) the extraction source budget, which now reaches 13 of 22 Notion sources; (4) the backfill; (5) Task 3 wording. The first re-file of each card after deploy misses its memoized How it wins judgment once, because the evidence packet hash changes. Record: `eval/curation/remediation-2026-09/raw/task2-side-by-side.md` (gitignored). See [the plan](superpowers/plans/2026-09-23-evidence-and-judgment-remediation.md).
