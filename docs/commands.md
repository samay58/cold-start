...
npm run qa:model-inputs -- --slug <slug> # tsx scripts/dump-model-inputs.ts (exact request each model stage would send; no model call)
...
npm run eval:how-it-wins:batch          # tsx scripts/how-it-wins-batch.ts (production-path How it wins corpus runner with a disk judgment cache, a hard --budget-usd spend cap, and the strategy-frequency gate; excludes the ten holdout slugs)
