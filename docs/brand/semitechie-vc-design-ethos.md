...
| Name | Value | Role |
|------|-------|------|
| Observatory Navy | `#06192C` | Primary brand field, extension shell, social graphics |
| Abyss Ink | `#020812` | Deepest background and icon badge edge |
| Lens Blue | `#1674FF` | Core signal, focused interaction, active citation |
| Wire Blue | `#5FB4FF` | Network lines, fine rules, radar grid |
| Ice White | `#EEF8FF` | Main icon stroke, inverse text, high-signal labels |
| Memo Parchment | `#FAFAF7` | Reading surface for long-form cards |
| Soft Graphite | `#5F625C` | Secondary text on light surfaces |
| Signal Amber | `#A8741F` | Conflict, warning, mixed-source state only |
...
- Icon badge can be circular because it is a mark, not a generic UI pill.
- Cards should feel like documents or instrument panes, not soft SaaS tiles.
- Lines should be hairline, radial, or connective. No decorative blobs.

## Product Translation

For Cold Start, the brand should appear in three layers:

1. **Toolbar icon and favicon:** dark observatory badge with white aperture and blue radar core.
...
- Treat `docs/brand/source/raw-icon-cold-start_RAW.png` as the visual source reference.
- App icons should preserve the radar iris detail from the raw image. Do not replace it with a flat drawn dot unless the size is too small to retain detail.
- The shipping PNGs must have alpha outside the dark badge. No white square and no checkerboard background.
- The 16px toolbar icon may be size-tuned for legibility, but 32px and up should retain the raw iris detail and fine radial grid.
- Keep `apps/extension/public/icons/cold-start-icon-master.png` and `apps/web/public/icons/cold-start-icon-master.png` as the generated master icons for product surfaces.
