...
type ColdStartCard = {
  ...
  // Long-form plain description behind the header's "(more)" affordance: what the company
  // makes and who uses it, how it makes money, where it sits among the players around it.
  // Descriptive only; generated by the block-enrichment worker and cited like every fact.
  expandedDescription?: { paragraphs: string[]; citationIds: string[] };
  funding: {
...
A Jev screen can run beside the judge in shadow mode (`HOW_IT_WINS_SCREEN=shadow`). It is not a judge provider request and sits outside the paid ledger, at about $0.002 a run. It records what it would have kept on the run trace and never changes a judgment or read. `HOW_IT_WINS_SCREEN=scoped`, off in production, runs the screen first as its own memoized step. The judge then rules only on Round 1's survivors, files the rest as insufficient evidence, and sends any current ruling that fails the Jev citation check to adjudication. A failed screen falls back to the full judge. A scoped verdict is filed under the screen's configuration rather than its output, so a re-file over unchanged evidence replays it without a new screen, and scoped and full verdicts never replay for each other. See [the layered screen](docs/superpowers/specs/2026-09-22-how-it-wins-layered-screen.md).
