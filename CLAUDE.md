...
- `apps/extension`: Chrome MV3 side panel built with Vite, CRXJS 2.7, React 19, and Framer Motion, plus an in-progress Firefox target (`docs/superpowers/plans/2026-07-13-firefox-port.md` is its source of truth). `manifest.config.ts` takes a `browser` parameter: the Chrome branch must keep its exact key order (dist manifest is diffed for byte stability), and the Firefox branch swaps `side_panel` for `sidebar_action`, declares `background.scripts` in source (CRXJS's firefox target crashes on a service_worker-only source manifest), and carries the gecko ID `cold-start@semitechie.vc`. `vite.firefox.config.ts` builds to `dist-firefox` via `npm run build:firefox -w @cold-start/extension` and must name `sidepanel.html` as a rollup input (CRXJS never discovers `sidebar_action.default_panel` as an entry). `src/background.ts` feature-detects `"sidePanel" in chrome`; on Firefox it calls `browser.sidebarAction.open()` synchronously first in the click handler (an await before it loses the user gesture), typed by the minimal `src/firefox.d.ts`. The first-90-seconds arc (intake, building, profile) is one shell: `src/CompanyArc.tsx` owns the phases, the persistent identity band in `src/CompanyHeader.tsx`, the early read placement, and the sealed Investor Lens row; while building, the assembling card itself is the progress surface: the header whisper in `CompanyArc.tsx` (copy from `whisperCopyFromEvents` in `src/research-progress.ts`) is the status voice, `src/Clippings.tsx` with `src/clipping-model.ts` renders live source clippings (classed with First Payoff's source-class taxonomy; thumbnails from `sources.image_url`, migration 0008, only for news/funding/customer-proof classes), and `src/SealInstrument.tsx` is the wax-seal progress object that inks up on real stage events and sets as the FILED stamp. `src/ResearchTrail.tsx` is only the quiet Details toggle plus the lazy `SourcePassInstrument` tree it opens (auto-open on attention). People on the profile get cited dossier hovercards (`src/CompanyHeader.tsx` with `src/SharedTooltip.tsx`), which carry the extension-tier person read. The active research-layer surface lives in `src/research-layer.ts`, `src/research-layer-motion.ts`, and `src/ResearchLayerPanel.tsx` (research layers plus card tray, composed under the arc); keep research module activation and pinning there rather than a separate analysis gate. First Payoff is the source-backed early read (the user-facing label is "Early read"; the code keeps the First Payoff name): core logic and schema live in `packages/core/src/first-payoff.ts`, the extension renders it inline via `src/ReadRegion.tsx` (receipt, substantive read, and withheld states while building; substantive only on the profile), with `src/first-payoff-events.ts` deriving it from progress events and `src/company-display.ts` owning the event-driven filing decision (`earlyReadState`). It states what the company does, who it serves, and the latest proof headline, each tied to citations, with explicit missing-proof and structured suppression reasons when no honest incremental claim survives. Headline newsworthiness classifies via the shared `headline` util in `packages/core`; keep that classifier in core, not duplicated in the extension. Source-class heuristics (docs/funding/customer-proof) likewise live in `packages/core/src/source-class.ts`, consumed by both First Payoff and the extension's `research-progress`, not re-implemented per surface. The investor lens (`src/investor-lens.ts`) renders as one memo card, the single synthesis surface: `investorReadForCard` builds the display and `InvestorReadCard` in `ResearchLayerPanel.tsx` renders Why care, The case (an If true / It breaks if tension), Timing, and Next question, each claim carrying a source posture and overflow filed behind "+N more" affordances; the research-layer deck beneath it carries only card-sourced layers (Who pays, Proof, Signals, Money, Comps, Product). Its voice comes from the `investor-taste-kernel` system prompt in `packages/llm/src/investor-taste-kernel.ts`. Person reads share that voice: the person-level `read` field on `personSchema` in `packages/core/src/card.ts` is synthesized by `packages/llm/src/person-read.ts` inside the contact-enrichment worker (gated by `PERSON_READS_ENABLED`, default on) and is stripped from the public card like the rest of synthesis.
